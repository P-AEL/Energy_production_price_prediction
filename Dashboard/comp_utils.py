...
class RebaseAPI:
  ...
  def get_submission_data(self,id_submitted,data):
  # Read the submission data
    url = f"{self.base_url}/challenges/{id_submitted}/submissions"
    resp = self.session.get(url,headers=self.headers, json=data, verify=self.ssl_verify)
    print(resp)
    print(resp.text)
    return resp.json()
